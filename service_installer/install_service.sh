...
if [ ! -f "python-apps-autostart.service" ]; then
    echo "Error: python-apps-autostart.service template not found!"
    exit 1
fi
...
# Use the template file and replace variables
sed "s/%i/$CURRENT_USER/g" python-apps-autostart.service > /tmp/$USER_SERVICE_NAME
